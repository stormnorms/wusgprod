...
class WebinarSysteemPreviewMails extends WebinarSysteem {
    ...
    public static function previewMails() {
        ...
        if (empty($_GET['run'])) {
            ...
            switch ($run) {
                case "_wswebinar_newreg":
            	    $mailingFunctions->SendMailtoAdmin("this is an preview email", $posts[0]->ID, $email, TRUE);
            	    break;
            	case "_wswebinar_regconfirm":
            	    $mailingFunctions->SendMailtoReader("this is an preview email", $email, $posts[0]->ID, TRUE);
            	    break;
