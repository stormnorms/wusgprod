 ...
 * @version $id$
 */
class WSAWeberServiceProvider implements WSOAuthServiceProvider {
...
class WSAWeberAPIBase {
    ...
    protected function readResponse($response, $url) {
        ...
        if (!empty($response['id'])) {
            return new WSAWeberEntry($response, $url, $this->adapter);
        } else if (array_key_exists('entries', $response)) {
            return new WSAWeberCollection($response, $url, $this->adapter);
        }
        return false;
    }
}
 ...
 * @version $id$
 */
class WSAWeberAPI extends WSAWeberAPIBase {
    ...
    public static function getDataFromAweberID($string) {
        list($consumerKey, $consumerSecret, $requestToken, $tokenSecret, $verifier) = WSAWeberAPI::_parseAweberID($string);
    ...
    public function setAdapter($adapter=null) {
        if (empty($adapter)) {
            $serviceProvider = new WSAWeberServiceProvider();
            $adapter = new WSOAuthApplication($serviceProvider);
