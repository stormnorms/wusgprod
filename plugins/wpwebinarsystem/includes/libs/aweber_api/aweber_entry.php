...
class WSAWeberEntry extends WSAWeberResponse {
    ...
    public function attrs() {
        ...
        if (!empty(WSAWeberAPI::$_collectionMap[$this->type])) {
            foreach (WSAWeberAPI::$_collectionMap[$this->type] as $child) {
    ...
    public function __get($value) {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new WSAWeberEntryDataArray($this->data[$value], $value, $this);
        ...
        throw new WSAWeberResourceNotImplemented($this, $value);
    }
    ...
    public function getParentEntry(){
        ...
        try {
            ...
            return new WSAWeberEntry($data, $url, $this->adapter);
        } catch (Exception $e) {
            return NULL;
        }
    }
    ...
    protected function _parseNamedOperation($data) {
        ...
        foreach($data as $entryData) {
            $results[] = new WSAWeberEntry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _getCollection($value) {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new WSAWeberCollection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value) {
        $this->_type();
        if (!empty(WSAWeberAPI::$_collectionMap[$this->type]) &&
            in_array($value, WSAWeberAPI::$_collectionMap[$this->type])) return true;
        return false;
    }
