<?php
if (!class_exists('WSCurlObject')) require_once('curl_object.php');
if (!class_exists('WSCurlResponse')) require_once('curl_response.php');
 ...
 * @version $id$
 */
interface WSOAuthServiceProvider {
 ...
 * @version $id$
 */
class WSOAuthApplication implements WSAWeberOAuthAdapter {
    public $debug = false;
    ...
    public function __construct($parentApp = false) {
        if ($parentApp) {
            if (!is_a($parentApp, 'WSOAuthServiceProvider')) {
                throw new Exception('Parent App must be a valid OAuthServiceProvider!');
            }
            $this->app = $parentApp;
        }
        $this->user = new WSOAuthUser();
        $this->curl = new WSCurlObject();
    }
    ...
    public function makeRequest($method, $url, $data=array()) {
        ...
        if (!$resp) {
            ...
            throw new WSAWeberAPIException($error, $url);
        }

        if($resp->headers['Status-Code'] >= 400) {
            $data = json_decode($resp->body, true);
            throw new WSAWeberAPIException($data['error'], $url);
        }
    ...
    private function _sendRequest($handle, $headers = array('Expect:')) {
        ...
        if ($resp) {
            return new WSCurlResponse($resp);
        }
