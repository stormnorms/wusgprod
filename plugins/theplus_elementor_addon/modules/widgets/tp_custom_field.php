	 ...
	 protected function render() {
		...
		switch ($cfkey_type) {
			...
			case "image":	
				...
				if(is_numeric($custom_field_val)){
					...
					if($image_type_select == 'lightbox') {
						$field_html .= '<a href="' . esc_url($custom_field_val) . '" title="' . $post_title . '">';
					}else if($image_type_select != '') {
						$field_html .= '<a href="' . esc_url($post_permalink) . '" title="' . $post_title . '">';
					}
			...
			case "link":	
				...
				}else if(!empty($cf_link_dynamic_text) && $settings['field_link_type'] == 'custom_field') {
					$field_html = '<a href="'.$custom_field_val.'" '.$this->get_render_attribute_string( 'tp-custom-field-class' ).' >'. $cf_link_dynamic_text .'</a>';
				}else{
			...
			case "date"  : 	
				...
				$field_html = sprintf( '<%1$s %2$s>%3$s</%1$s>', $settings['header_tag'], $this->get_render_attribute_string( 'tp-custom-field-class' ), do_shortcode($field_html) );
			...
			default:
				$field_html = sprintf( '<%1$s %2$s>%3$s</%1$s>', $settings['header_tag'], $this->get_render_attribute_string( 'tp-custom-field-class' ), do_shortcode($custom_field_val) );
				break;
		}
