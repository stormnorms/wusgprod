						...
						foreach($settings["listing_content"] as $item) {
							...
							if(!empty($item['on_hover_img_reveal_switch']) && $item['on_hover_img_reveal_switch']=='yes'){
								if(!empty($item['adv_hover_img_reveal_style'])){
									...
									if(! empty( $item['text_link']['url'] )){ 
										$loop_img_reveal_open .='<a '.$this->get_render_attribute_string( "loop_typo_link".$i ).' class="block__title" data-img="'.$on_hover_img_source.'">';
									}else{
					...
					if(!empty($advanced_typography_text)){
						...
						if($settings['marquee_switch']=='yes'){
							...
							if (!empty($settings['text_link']['url'])){								
								...
								$adv_typ_text .= '<a id="'.esc_attr($uidat).'" '.$this->get_render_attribute_string( "typo_link_a" ).' class="text-content-block '.$adv_typ_text1.' '.esc_attr($circular_text_switch).' '.esc_attr($stroke_switch).' '.esc_attr($background_based_text_switch).' '.esc_attr($img_gif_ovly_txtimg_switch).' '.esc_attr($on_hover_img_reveal_switch).' '.esc_attr($text_continuous_animation).' '.esc_attr($animated_class).'" '.$data_attr.' style="'.$text_write_mode.$mix_blend_attr.' " '.$circular_attr.' '.$animation_attr.' data-img="'.$adv_typ_text2.'">'.htmlspecialchars_decode($loop_text).'</a>';						
							}else{
								$adv_typ_text ='';
								if(!empty($settings['on_hover_img_reveal_switch']) && $settings['on_hover_img_reveal_switch']=='yes'){
									if(!empty($settings['adv_hover_img_reveal_style'])){
